...
function init() {
  ...
  // ----------------------------------------------------
  // 🧠 ZEPRA BRIDGE (CHAT EXPORT ENGINE)
  // ----------------------------------------------------
  const ZEPRA_BRIDGE = {
    cleanHTML: (html) => {
...
`;
  ...
  function handleBrainBridge() {
    ...
    if (scraperKey) {
      showNotification('🧠 Bridging conversation history...', 'info');
      ...
      setTimeout(() => {
        try {
          ...
          showNotification('✅ Export Successful! Upload this file to any AI platform to continue your chat.', 'success', 6000);
        } catch (err) {
          console.error(err);
      ...
      <div style="text-align:center; padding: 10px;">
        ...
        <div style="background: rgba(15,23,42,0.5); border:1px solid #334155; border-radius: 12px; padding: 16px; text-align: left;">
          ...
          <div style="display:flex; align-items:center; gap:10px; color:#e2e8f0; font-size:14px; margin-bottom:8px;">
            <span style="color:#10a37f;">●</span> ChatGPT (chatgpt.com)
          </div>
          <div style="display:flex; align-items:center; gap:10px; color:#e2e8f0; font-size:14px; margin-bottom:8px;">
            <span style="color:#10a37f;">●</span> Gemini (gemini.google.com)
          </div>
          <div style="display:flex; align-items:center; gap:10px; color:#e2e8f0; font-size:14px; margin-bottom:8px;">
            <span style="color:#4d6bfe;">●</span> DeepSeek (chat.deepseek.com)
          </div>
          <div style="display:flex; align-items:center; gap:10px; color:#e2e8f0; font-size:14px; margin-bottom:8px;">
            <span style="color:#22b3ba;">●</span> Perplexity (perplexity.ai)
          </div>
          <div style="display:flex; align-items:center; gap:10px; color:#e2e8f0; font-size:14px; margin-bottom:8px;">
            <span style="color:#d97757;">●</span> Claude (claude.ai)
          </div>
          <div style="display:flex; align-items:center; gap:10px; color:#e2e8f0; font-size:14px; margin-bottom:8px;">
             <span style="color:#815df2;">●</span> ZAI (ChatGLM)
          </div>
          <div style="display:flex; align-items:center; gap:10px; color:#e2e8f0; font-size:14px; margin-bottom:8px;">
             <span style="color:#6B4BFF;">●</span> Poe (poe.com)
          </div>
          <div style="display:flex; align-items:center; gap:10px; color:#e2e8f0; font-size:14px; margin-bottom:8px;">
             <span style="color:#F5A623;">●</span> Mistral AI (chat.mistral.ai)
          </div>
        </div>
...
async function saveToLibraryDirect(additionalNote = '') {
  try {
    ...
    if (response?.ok) {
      showLibraryToast('✓ Website saved to library!', 'success');
    } else {
  ...
  async function enhancePrompt(field, wand) {
    ...
    try {
      ...
      if (promptEnhanceConfirm) {
        ...
        showLibraryToast('✨ Prompt Enhanced', 'success');
    ...
    modal.innerHTML = `
      ...
      <div class="zepra-enhance-modal">
        <div class="enhance-content">
            ...
            <div class="enhance-col">
                <h3 style="color:#34d399">✨ Enhanced (Super Prompt)</h3>
...
function createZepraHeader() {
  ...
  const renderDockContent = () => {
    dock.innerHTML = '';
    dock.appendChild(badge);
    dock.appendChild(createBtn('🔙', 'Back', () => { window.close(); }));

    dock.appendChild(createBtn('📌', 'Dock', async () => {
      try {
        const res = await chrome.runtime.sendMessage({
          type: 'ADD_TO_PROFILE',
          url: window.location.href,
        ...
        dock.innerHTML = `<span style="color:#00ff88; font-weight:bold; padding:10px;">✅ Added to My Toolkit!</span>`;
        setTimeout(() => {
        ...
        console.error(e);
        dock.innerHTML = `<span style="color:#ff4444; padding:10px;">❌ Failed!</span>`;
        setTimeout(renderDockContent, 2000);
      }
    }, '#00ff88'));

    dock.appendChild(createBtn('❌', 'Close', () => { window.close(); }, '#ff4444'));
  };
