...
class ZepraDubBridge {
    constructor() {
        this._isTopFrame = window.top === window.self;
        if (!this._isTopFrame) return;
    ...
    startSession({ sourceLang, targetLang } = {}) {
        this.activeVideo = this.activeVideo || document.querySelector('video');
