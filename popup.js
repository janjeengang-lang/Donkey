...
const els = {
  ...
  navClipboard: document.getElementById('navClipboard'),
  navCommand: document.getElementById('navCommand'),
  navMarket: document.getElementById('navMarket'),
  navMemory: document.getElementById('navMemory'),
  navPersona: document.getElementById('navPersona'),
};
...
els.navClipboard?.addEventListener('click', () => {
  chrome.tabs.create({ url: chrome.runtime.getURL('clipboard.html') });
});
els.navCommand?.addEventListener('click', () => {
  chrome.tabs.create({ url: chrome.runtime.getURL('command_center.html') });
});
els.navMarket?.addEventListener('click', () => {
  chrome.tabs.create({ url: chrome.runtime.getURL('marketplace.html') });
});
els.navMemory?.addEventListener('click', () => {
...
async function handleMode(mode) {
  ...
  try {
    ...
    const prompt = buildPrompt('auto', questionText, ctx, thinking, settings);
...
function buildPrompt(mode, question, context, thinking) {
  ...
  const personaSettings = arguments.length >= 5 ? arguments[4] : {};
  const header = buildPromptHeader({
    personaEnabled: personaSettings?.personaEnabled,
    personaName: personaSettings?.personaActiveName,
    personaPrompt: personaSettings?.personaActivePrompt,
    humanErrorRate: personaSettings?.humanErrorRate,
    mode: 'auto'
  });
  let rules = `${header}You are answering a survey question. Use prior context if helpful and choose answers that keep the participant qualified for the survey.\nSTRICT OUTPUT RULES:\n`;
  if (thinking) {
...
async function saveContext(entry) { const list = await getContext(); list.push({ ...entry, ts: Date.now() }); await chrome.storage.local.set({ contextQA: list }); await chrome.runtime.sendMessage({ type: 'SURVEY_MEMORY_UPDATED', entries: list.slice(-50) }); }
