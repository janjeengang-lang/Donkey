    ...
    try {
      switch (message.type) {
        ...
        case 'DUB_SCOUT_READY': {
          ...
          let mainSession = Array.from(dubbingSessions.values()).find(s => s.scoutTabId === sender.tab.id);
          if (!mainSession && dubbingSessions.size === 1) {
            mainSession = Array.from(dubbingSessions.values())[0];
            mainSession.scoutTabId = sender.tab.id;
          }
          if (mainSession) {
        ...
        case 'DUB_SCOUT_TRANSLATION': {
          if (!sender.tab?.id) break;
          let session = Array.from(dubbingSessions.values()).find(s => s.scoutTabId === sender.tab.id);
          if (!session && dubbingSessions.size === 1) {
            session = Array.from(dubbingSessions.values())[0];
            session.scoutTabId = sender.tab.id;
          }
          if (!session) break;
