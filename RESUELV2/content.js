...
function init() {
  ...
  let promptEnhanceEnabled = false;
  let promptEnhanceSettings = { model: '', style: 'professional', format: 'paragraph', confirm: 'no' };

  async function loadPromptEnhanceSettings() {
    const settings = await chrome.storage.local.get([
      'promptEnhanceEnabled',
      'promptEnhanceModel',
      'promptEnhanceStyle',
      'promptEnhanceFormat',
      'promptEnhanceConfirm'
    ]);
    promptEnhanceEnabled = !!settings.promptEnhanceEnabled;
    promptEnhanceSettings = {
      model: settings.promptEnhanceModel || '',
      style: settings.promptEnhanceStyle || 'professional',
      format: settings.promptEnhanceFormat || 'paragraph',
      confirm: settings.promptEnhanceConfirm || 'no'
    };
  }

  chrome.storage.onChanged.addListener((chg, area) => {
    if (area === 'local' && (chg.promptEnhanceEnabled || chg.promptEnhanceModel || chg.promptEnhanceStyle || chg.promptEnhanceFormat || chg.promptEnhanceConfirm)) {
      loadPromptEnhanceSettings();
    }
  });
  loadPromptEnhanceSettings();

  function isEnhanceableField(el) {
    if (!el) return false;
    if (el.isContentEditable) return true;
    if (el.getAttribute && ['textbox', 'searchbox'].includes(el.getAttribute('role'))) return true;
    if (el.tagName === 'TEXTAREA') return true;
    if (el.tagName === 'INPUT') {
      const t = (el.type || '').toLowerCase();
      return !['password', 'number', 'email', 'tel', 'url', 'search', 'file', 'checkbox', 'radio', 'date', 'datetime-local', 'time', 'color'].includes(t);
    }
    return false;
  }

  function addEnhanceIcon(el) {
    removeEnhanceIcon();
    if (!promptEnhanceEnabled || !isEnhanceableField(el)) return;
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'zepra-enhance-btn';
    btn.textContent = 'AI';
    btn.title = 'Enhance prompt';
    btn.style.cssText = 'position:fixed;background:#0f172a;border:1px solid #38bdf8;color:#38bdf8;font-size:11px;font-weight:700;border-radius:8px;padding:2px 6px;cursor:pointer;z-index:2147483647;box-shadow:0 6px 16px rgba(15,23,42,0.45);';
    document.body.appendChild(btn);

    const updatePosition = () => {
      if (!document.contains(el) || !document.contains(btn)) return;
      const rect = el.getBoundingClientRect();
      if (!rect.width && !rect.height) return;
      const dir = getComputedStyle(el).direction === 'rtl' ? 'rtl' : 'ltr';
      const btnRect = btn.getBoundingClientRect();
      const gap = 8;
      let left = dir === 'rtl' ? rect.left + gap : rect.right - btnRect.width - gap;
      let top = rect.top + rect.height / 2 - btnRect.height / 2;
      left = Math.max(8, Math.min(window.innerWidth - btnRect.width - 8, left));
      top = Math.max(8, Math.min(window.innerHeight - btnRect.height - 8, top));
    ...
    btn.addEventListener('click', async (e) => {
      ...
      try {
        const res = await chrome.runtime.sendMessage({
          ...
          text: value,
          options: {
            model: promptEnhanceSettings.model || undefined,
            style: promptEnhanceSettings.style,
            format: promptEnhanceSettings.format
          }
        });
        if (!res?.ok) throw new Error(res?.error || 'Enhance failed');
        const enhanced = res.enhanced || value;
        if (promptEnhanceSettings.confirm === 'yes') {
          const ok = confirm('Replace text with enhanced version?');
          if (!ok) { btn.textContent = 'AI'; return; }
        }
        if (typeof el.value === 'string') {
          el.value = enhanced;
        } else if (el.isContentEditable) {
          el.innerText = enhanced;
        } else {
          el.textContent = enhanced;
        }
        el.dispatchEvent(new Event('input', { bubbles: true }));
      } catch (err) {
        showNotification('Prompt enhancement failed: ' + (err?.message || err), 'error');
      } finally {
        btn.textContent = 'AI';
      }
    });
    STATE.promptEnhance = { btn, updatePosition, onScroll, onResize };
  }

  function removeEnhanceIcon() {
    const pe = STATE.promptEnhance;
    if (pe) {
      pe.btn.remove();
      window.removeEventListener('scroll', pe.onScroll, true);
      window.removeEventListener('resize', pe.onResize);
      STATE.promptEnhance = null;
    }
  }
  ...
  function resolveFocusTarget(target, path) {
    const chain = Array.isArray(path) ? path : [];
    const candidate = chain.find((el) => isEnhanceableField(el)) || target;
    return candidate;
  }

  document.addEventListener('focusin', (e) => {
    const target = resolveFocusTarget(e.target, e.composedPath?.());
    STATE.lastFocused = target;
    showFieldIcon(target);
    addEnhanceIcon(target);
  });
  document.addEventListener('focusout', () => { removeFieldIcon(); removeEnhanceIcon(); });
    ...
    menu.innerHTML = `
      ...
      <nav class="sidebar-nav">
        <ul>
          ...
          <li><a href="#" data-action="add-web-lib"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/><path d="M12 6h.01"/><path d="M12 10h.01"/><path d="M12 14h.01"/></svg><span>Add to Library</span></a></li>
          <li><a href="#" data-action="clipboard-mem"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1" ry="1"/></svg><span>Clipboard Memory</span></a></li>
  ...
  async function handleBubbleAction(action) {
    switch (action) {
      ...
      case 'add-web-lib':
        ...
        break;
      case 'clipboard-mem':
        chrome.runtime.sendMessage({ type: 'OPEN_PAGE', page: 'clipboard.html' });
        break;
    }
  }
  ...
  function showAddWebsiteModal() {
    const content = `
      <style>
        .awm-wrap { display: flex; flex-direction: column; gap: 12px; min-width: 300px; }
        .awm-textarea { width: 100%; height: 80px; background: rgba(0,0,0,0.2); border: 1px solid rgba(148,163,184,0.3); border-radius: 8px; padding: 10px; color: #fff; resize: vertical; margin-bottom: 10px; font-family: inherit; }
        .awm-textarea:focus { outline: none; border-color: #38bdf8; }
        .awm-btn { width: 100%; padding: 10px; background: #38bdf8; border: none; border-radius: 8px; color: #0f172a; font-weight: bold; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; }
        .awm-btn:hover { background: #0ea5e9; }
        .awm-hint { font-size: 11px; color: #94a3b8; text-align: center; margin-top: 4px; }
      </style>
      <div class="awm-wrap">
        <div style="color:#e2e8f0;font-size:14px;margin-bottom:4px;">Add Note (Optional)</div>
        <textarea id="awmNote" class="awm-textarea" placeholder="E.g., credentials, usage notes..."></textarea>
        <button id="awmSave" class="awm-btn">
        ...
        <div class="awm-hint">Zepra will analyze page content automatically.</div>
      </div>
    `;
    const modal = _safeCreateStyledModal('Add to Library', content);

    modal.querySelector('#awmSave').addEventListener('click', async () => {
      const btn = modal.querySelector('#awmSave');
      const note = modal.querySelector('#awmNote').value;
      btn.innerHTML = 'Analyzing...';
      btn.disabled = true;

      try {
        const content = document.body.innerText.slice(0, 6000);
        const res = await chrome.runtime.sendMessage({
  ...
  document.addEventListener('copy', async () => {
    try {
      ...
      if (selection && selection.trim().length > 0) {
        const { clipboardEnabled } = await chrome.storage.local.get('clipboardEnabled');
        if (clipboardEnabled) {
      ...
      if (modal) {
        ...
        if (useReason) {
          ...
          if (split) {
            ...
            if (reasonEl) {
              reasonEl.innerHTML = parseMarkdown(reason || 'No reason provided');
...
.zn-pill.ghost{background:linear-gradient(135deg,#1f2937,#0f172a);border-color:rgba(148,163,184,0.35)}
  ...
  async function createNoteFromPayload(payload, rawExtras = {}) {
    const { notesGenerateModel = '' } = await chrome.storage.local.get('notesGenerateModel');
    const res = await chrome.runtime.sendMessage({
      type: 'GENERATE_NOTE',
      payload,
      options: { model: notesGenerateModel || '' }
    });
      ...
      <div id="${id}" class="artifact-widget" style="border:1px solid #10B981; background:#0f172a; border-radius:12px; margin:15px 0; overflow:hidden; box-shadow:0 10px 15px -3px rgba(0,0,0,0.3);">
        <div class="artifact-toolbar" style="background:#111827; padding:10px 15px; border-bottom:1px solid #334155; display:flex; justify-content:space-between; align-items:center;">
          ...
          <div style="display:flex; gap:8px;">
            <button class="artifact-btn-fullscreen" onclick="this.closest('.artifact-widget').classList.toggle('maximized')" style="background:#1f2937; border:1px solid #374151; color:#94a3b8; padding:4px 8px; border-radius:6px; cursor:pointer; font-size:11px;" title="Toggle Fullscreen">⛶</button>
            <button class="artifact-btn-download" onclick="const h = this.closest('.artifact-widget').querySelector('iframe').getAttribute('srcdoc'); const b = new Blob([h], {type:'text/html'}); const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href=u; a.download='artifact.html'; a.click();" style="background:#10B981; border:none; color:white; padding:4px 10px; border-radius:6px; cursor:pointer; font-size:11px; font-weight:bold;">DOWNLOAD</button>
          </div>
        </div>
        <div class="artifact-preview" style="background:white; height:350px; position:relative;">
          <iframe srcdoc="${escapedHtml}" style="width:100%; height:100%; border:none;"></iframe>
        </div>
      </div>
    `;
  }
...
AI RESPONSE:`;
  ...
  function handleSelection(e) {
    ...
    if (active && (active.tagName === 'TEXTAREA' || active.tagName === 'INPUT')) {
      ...
      if (end > start) {
        ...
        const slice = val.slice(start, end).trim();
        if (slice) {
          const rect = active.getBoundingClientRect();
          showSelectionButton(rect, slice, { anchor: 'field' });
          return;
        }
      }
    }
  ...
  function showSelectionButton(rect, text, opts = {}) {
    removeSelectionButton();
    ...
    const anchorWidth = Math.max(20, rect.right - rect.left);
    let left = rect.left + anchorWidth / 2 - btnWidth / 2;
    let top = rect.top - btnHeight - 8;
    left = Math.min(window.innerWidth - btnWidth - 10, Math.max(10, left));
    top = Math.min(window.innerHeight - btnHeight - 10, Math.max(10, top));
    btn.style.cssText = `position:fixed;left:${left}px;top:${top}px;z-index:2147483647;background:#23272b;color:#39ff14;padding:4px 10px;border-radius:8px;font-size:12px;box-shadow:0 0 12px rgba(57,255,20,0.35);cursor:pointer;transition:transform 0.2s;`;
