...
async function generateExport() {
  ...
  const options = {
    style,
    tone: els.exportTone?.value || 'creative',
    direction: dir,
  ...
  showLoading(true);
  try {
    // Step 1: Ask AI to generate complete HTML design
    ...
    if (!aiResponse?.ok) throw new Error(aiResponse?.error || 'AI generation failed');

    // Step 2: Save the AI-generated HTML for export page
    const payload = {
...
async function deleteNote(id) {
  ...
  await chrome.storage.local.set({ zepraNotes: updated });
  state.notes = updated;
...
function showLoading(val) {
  ...
  const steps = els.loading?.querySelectorAll('[data-step]');
