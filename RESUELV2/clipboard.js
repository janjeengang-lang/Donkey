...
function buildCardAnalysis(item) {
    ...
    const summary = item.aiSummary ? `<div class="card-analysis-summary">${htmlEscape(item.aiSummary)}</div>` : '';
    const points = (item.keyPoints || []).slice(0, 3).map(pt => `<li>${htmlEscape(pt)}</li>`).join('');
...
function renderSafeText(text) {
    return htmlEscape(String(text || '')).replace(/\n/g, '<br>');
}
